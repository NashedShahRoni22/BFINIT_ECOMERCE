      ...
      <td className="px-4 py-2 text-xs font-medium whitespace-nowrap">
        {!store?.charges_enabled || !store?.payouts_enabled ? (
          <button
            ...
            disabled={isPending}
            className={`inline-flex items-center gap-0.5 rounded-full px-2 py-1 text-white transition-all duration-200 ease-in-out ${isPending ? "bg-[#8a82ff]" : "cursor-pointer bg-[#675dff] hover:bg-[#4c42db]"}`}
          >
